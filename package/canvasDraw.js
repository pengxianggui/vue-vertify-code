...
function drawPath(ctx, x, y, operation, l, r) {
    ctx.beginPath()
    ctx.moveTo(x, y)
...
class Drawer {
    ...
    side = 42; // 滑块边长
    radius = 9; // 滑块半径
    L = this.side + this.radius * 2 + 3; // 滑块实际边长

    constructor({el, width, height, side, radius, onSuccess, onFail, onRefresh}) {
    ...
    draw(img) {
        const {width, height, L, side, radius} = this
        // 随机位置创建拼图形状
        this.x = getRandomNumberByRange(L + 10, width - (L + 10))
        this.y = getRandomNumberByRange(10 + radius * 2, height - (L + 10))
        drawPath(this.canvasCtx, this.x, this.y, 'fill', side, radius)
        drawPath(this.blockCtx, this.x, this.y, 'clip', side, radius)
    ...
    reset() {
        ...
        this.img.setSrc(getRandomImgSrc(width, height))
    }
}

export default {
    init: function (opts) {
        return new Drawer(opts).init()
    }
}
